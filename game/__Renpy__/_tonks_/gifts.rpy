

#Tonks Gift Responses

label give_ton_gift(gift_item):
    hide screen tonks_main
    with d5
    call ton_main(xpos="mid",ypos="base",trans="d5")

    if gift_item == lollipop_ITEM:
        call give_gift(">You give the lollipop to Tonks...",gift_item)
        #Add text

    if gift_item == chocolate_ITEM:
        call give_gift(">You give the chocolate to Tonks...",gift_item)
        #Add text

    if gift_item == plush_owl_ITEM:
        call give_gift(">You give the plush owl to Tonks...",gift_item)
        #Add text

    if gift_item == butterbeer_ITEM:
        call give_gift(">You give the butterbeer to Tonks...",gift_item)
        #Add text

    if gift_item == science_mag_ITEM:
        call give_gift(">You give the educational magazine to Tonks...",gift_item)
        #Add text

    if gift_item == girls_mag_ITEM:
        call give_gift(">You give the girls magazine to Tonks...",gift_item)
        #Add text

    if gift_item == adult_mag_ITEM:
        call give_gift(">You give the adult magazine to Tonks...",gift_item)
        #Add text

    if gift_item == porn_mag_ITEM:
        call give_gift(">You give the porn magazine to Tonks...",gift_item)
        #Add text

    if gift_item == krum_poster_ITEM:
        call give_gift(">You give the poster to Tonks...",gift_item)
        #Add text

    if gift_item == sexy_lingerie_ITEM:
        call give_gift(">You give the lingerie to Tonks...",gift_item)
        #Add text

    if gift_item == pink_condoms_ITEM:
        call give_gift(">You give the condoms to Tonks...",gift_item)
        #Add text

    if gift_item == vibrator_ITEM:
        call give_gift(">You give the vibrator to Tonks...",gift_item)
        #Add text

    if gift_item == anal_lube_ITEM:
        call give_gift(">You give the anal lube to Tonks...",gift_item)
        #Add text

    if gift_item == ballgag_and_cuffs_ITEM:
        call give_gift(">You give the handcuffs to Tonks...",gift_item)
        #Add text

    if gift_item == anal_plugs_ITEM:
        call give_gift(">You give the anal plugs to Tonks...",gift_item)
        #Add text

    if gift_item == testral_strapon_ITEM:
        call give_gift(">You give the strap-on to Tonks...",gift_item)
        #Add text

    if gift_item == broom_2000_ITEM:
        call give_gift(">You give the broom to Tonks...",gift_item)
        #Add text

    if gift_item == sexdoll_ITEM:
        call give_gift(">You give the doll to Tonks...",gift_item)
        #Add text


    hide screen tonks_main
    with d5
    call ton_main(xpos="base",ypos="base",trans="d5")

    return
