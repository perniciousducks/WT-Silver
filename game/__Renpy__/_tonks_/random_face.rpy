...
label set_ton_face(face="random"):
    ...
    if face != "random":
        ...
        if face in ["neutral"]:
            ...
            $ temp_mouth    = renpy.random.choice(["base","horny","open","open_wide_tongue"])
        elif face in ["eyes"]:
            $ temp_eyes     = renpy.random.choice(["base","wide"])
        elif face in ["eyebrows"]:
            $ temp_eyebrows = renpy.random.choice(["angry","base","raised","wide","worried"])
        elif face in ["pupils"]:
            $ temp_pupils   = renpy.random.choice(["ahegao","ahegao_intense","down","L","mid","R","up","wide"])

        else:
            pass

    else:
        call set_ton_face("happy")
