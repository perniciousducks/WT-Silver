

###HARRY POTTER ANIMATIONS###

    
### HERMIONE GIVING GENIE A HANDJOB ###
image handjob_ani:
    "images/animation/12_handjob_01.png"
    pause.1
    "images/animation/12_handjob_02.png"
    pause.1
    "images/animation/12_handjob_03.png"
    pause.1
    "images/animation/12_handjob_04.png"
    pause.1
    "images/animation/12_handjob_05.png"
    pause.1
    "images/animation/12_handjob_06.png"
    pause.1
    repeat

### HERMIONE HANDJOB CUMMING UNDER THE SHIRT ###
image undershirt_cum_ani:
    "images/animation/14_finish_01.png"
    pause 1
    "images/animation/14_finish_02.png"
    pause.1
    "images/animation/14_finish_03.png"
    pause.1
    "images/animation/14_finish_04.png"
    pause.1
    "images/animation/14_finish_05.png"
    pause.1
    "images/animation/14_finish_06.png"
    pause.1
    "images/animation/14_finish_07.png"
    pause.1
    "images/animation/14_finish_08.png"
    pause.1
    "images/animation/14_finish_09.png"
    pause.1
    "images/animation/14_finish_10.png"
    pause 2
    "images/animation/14_finish_11.png"
    pause.1
    "images/animation/14_finish_12.png"
    pause.1
    "images/animation/14_finish_13.png"
    pause.1
    repeat

### HERMIONE HANDJOB CUMMING ON THE SHIRT ###
image on_shirt_cum_ani:
    "images/animation/15_cum_00.png"
    pause.1
    "images/animation/15_cum_01.png"
    pause.1
    "images/animation/15_cum_02.png"
    pause.1
    "images/animation/15_cum_03.png"
    pause.1
    "images/animation/15_cum_04.png"
    pause.1
    "images/animation/15_cum_05.png"
    pause.1
    "images/animation/15_cum_06.png"
    pause.1
    "images/animation/15_cum_07.png"
    pause.1
    "images/animation/15_cum_08.png"
    pause.1
    "images/animation/15_cum_09.png"
    pause.1
    "images/animation/15_cum_10.png"
    pause.1
    "images/animation/15_cum_11.png"
    pause.1
    "images/animation/15_cum_12.png"
    pause.1
    "images/animation/15_cum_13.png"
    pause.1
    "images/animation/15_cum_14.png"
    pause.1
    "images/animation/15_cum_15.png"
    pause.1
    "images/animation/15_cum_16.png"
    pause.1
    "images/animation/15_cum_17.png"
    pause.1
    "images/animation/15_cum_18.png"
    pause.1
    "images/animation/15_cum_19.png"
    pause.1
    "images/animation/15_cum_20.png"
    pause.1
    "images/animation/15_cum_21.png"
    pause 1
    "images/animation/15_cum_22.png"
    pause.1
    "images/animation/15_cum_21.png"
    pause.1
    "images/animation/15_cum_22.png"
    pause .1
    "images/animation/15_cum_21.png"
    pause 1
    "images/animation/15_cum_23.png"
    pause.1
    "images/animation/15_cum_24.png"
    pause.1
    "images/animation/15_cum_25.png"
    pause.1
    "images/animation/15_cum_00.png"
    pause.7
    repeat

### HERMIONE GIVING A KISS TO GENIE'S COCK ###
image kiss_ani:
    "images/animation/13_kiss_01.png"
    pause.1
    "images/animation/13_kiss_02.png"
    pause.1
    "images/animation/13_kiss_03.png"
    pause.1
    "images/animation/13_kiss_04.png"
    pause.1
    "images/animation/13_kiss_05.png"
    pause.1
    "images/animation/13_kiss_06.png"
    pause.1
    "images/animation/13_kiss_07.png"
    pause.1
    "images/animation/13_kiss_08.png"
    pause.1
    "images/animation/13_kiss_09.png"
    pause.1
    "images/animation/13_kiss_10.png"
    pause.1
    "images/animation/13_kiss_11.png"
    pause.1
    "images/animation/13_kiss_12.png"
    pause.1
    repeat

### HERMIONE HANDJOB KISS DRINKING CUM FROM COCK ###
image kiss_cum_ani:
    "images/animation/16_cum_01.png"
    pause.1
    "images/animation/16_cum_02.png"
    pause.1
    "images/animation/16_cum_03.png"
    pause.1
    "images/animation/16_cum_04.png"
    pause.1
    "images/animation/16_cum_05.png"
    pause.1
    "images/animation/16_cum_06.png"
    pause.1
    "images/animation/16_cum_07.png"
    pause.1
    repeat



#==========SEX===========#
image sex_ani:#-----------------### HERMIONE SEX ###
    "images/animation/21_sex_01.png"
    pause.1
    "images/animation/21_sex_02.png"
    pause.1
    "images/animation/21_sex_03.png"
    pause.1
    "images/animation/21_sex_04.png"
    pause.1
    "images/animation/21_sex_05.png"
    pause.1
    "images/animation/21_sex_06.png"
    pause.1
    "images/animation/21_sex_07.png"
    pause.1
    repeat

image sex2_ani:#----------------### HERMIONE FAST SPEED SEX ###
    "images/animation/21_sex_01.png"
    pause.05
    "images/animation/21_sex_02.png"
    pause.05
    "images/animation/21_sex_03.png"
    pause.05
    "images/animation/21_sex_04.png"
    pause.05
    "images/animation/21_sex_05.png"
    pause.05
    "images/animation/21_sex_06.png"
    pause.05
    "images/animation/21_sex_07.png"
    pause.05
    repeat

image sex_slow_ani:#------------### HERMIONE SEX LOW SPEED ###
    "images/animation/21_sex_01.png"
    pause.15
    "images/animation/21_sex_02.png"
    pause.15
    "images/animation/21_sex_03.png"
    pause.15
    "images/animation/21_sex_04.png"
    pause.15
    "images/animation/21_sex_05.png"
    pause.15
    "images/animation/21_sex_06.png"
    pause.15
    "images/animation/21_sex_07.png"
    pause.15
    repeat

image sex_cum_out_ani:#---------### HERMIONE SEX CUM OUTSIDE ###
    "images/animation/22_cum_01.png"
    pause.1
    "images/animation/22_cum_02.png"
    pause.1
    "images/animation/22_cum_03.png"
    pause.1
    "images/animation/22_cum_04.png"
    pause.1
    "images/animation/22_cum_05.png"
    pause.1
    "images/animation/22_cum_06.png"
    pause.1
    "images/animation/22_cum_07.png"
    pause.1
    "images/animation/22_cum_08.png"
    pause.1
    "images/animation/22_cum_09.png"
    pause.1
    "images/animation/22_cum_10.png"
    pause.1
    "images/animation/22_cum_11.png"
    pause.1
    "images/animation/22_cum_12.png"
    pause.1
    "images/animation/22_cum_13.png"
    pause.1
    "images/animation/22_cum_14.png"
    pause.1
    "images/animation/22_cum_15.png"
    pause.1
    "images/animation/22_cum_16.png"
    pause.1
    "images/animation/22_cum_17.png"
    pause.1
    "images/animation/22_cum_18.png"
    pause.1
    "images/animation/22_cum_19.png"
    pause 2
    "images/animation/22_cum_20.png"
    pause.1
    "images/animation/22_cum_21.png"
    pause.1
    "images/animation/22_cum_22.png"
    pause.1
    "images/animation/22_cum_23.png"
    pause.1
    repeat

image sex_cum_out_blink_ani:#---### HERMIONE SEX CUM OUTSIDE BLINKING ###
    "images/animation/22_cum_19.png"
    pause 1
    "images/animation/22_cum_24.png" #closed
    pause.1
    "images/animation/22_cum_19.png"
    pause.1
    "images/animation/22_cum_24.png" #closed
    pause.1
    "images/animation/22_cum_19.png"
    pause 2
    "images/animation/22_cum_24.png" #closed
    pause.1
    "images/animation/22_cum_19.png"
    pause 2
    repeat

image sex_cum_in_ani:#----------### HERMIONE SEX CUM INSIDE ###
    "images/animation/23_cum_01.png"
    pause.1
    "images/animation/23_cum_02.png"
    pause.1
    "images/animation/23_cum_03.png"
    pause.1
    "images/animation/23_cum_04.png"
    pause.1
    "images/animation/23_cum_05.png"
    pause.1
    "images/animation/23_cum_06.png"
    pause.1
    "images/animation/23_cum_07.png"
    pause.1
    "images/animation/23_cum_08.png"
    pause.1
    "images/animation/23_cum_09.png"
    pause.1
    "images/animation/23_cum_10.png"
    pause.1
    "images/animation/23_cum_11.png"
    pause.1
    "images/animation/23_cum_12.png"
    pause.1
    "images/animation/23_cum_13.png"
    pause.1
    "images/animation/23_cum_14.png"
    pause.1
    "images/animation/23_cum_15.png"
    pause.1
    "images/animation/23_cum_16.png"
    pause.1
    "images/animation/23_cum_17.png"
    pause.1
    "images/animation/23_cum_18.png"
    pause.1
    "images/animation/23_cum_19.png"
    pause 3
    "images/animation/23_cum_20.png"
    pause.1
    "images/animation/23_cum_21.png"
    pause.1
    "images/animation/23_cum_22.png"
    pause.1
    "images/animation/23_cum_23.png"
    pause.1
    repeat 

image pause_sex:#---------------### HERMIONE PAUSE SEX ###  
    "images/animation/21_sex_03.png"
    
    

### FAVORS ###
### GROPINGS ###
image groping_01: #Genie groping Hermione under her skirt. Hermione is facing Genie.
    "images/animation/grope_01.png"
    pause.2
    "images/animation/grope_02.png"
    pause.2
    "images/animation/grope_03.png"
    pause.5
    "images/animation/grope_02.png"
    pause.2
    "images/animation/grope_01.png"
    pause.2
    repeat

image groping_02: #Genie groping Hermione under her skirt. Hermione's behind is facing Genie.
    "images/animation/grope_b_01.png"
    pause.2
    "images/animation/grope_b_02.png"
    pause.2
    "images/animation/grope_b_03.png"
    pause.5
    "images/animation/grope_b_02.png"
    pause.2
    "images/animation/grope_b_01.png"
    pause.2
    repeat


image groping_01_blinking: #Animation of Hermione blinking her eyes.
    "images/animation/00.png"
    pause.1
    "images/animation/grope_04.png"
    pause.1
    "images/animation/00.png"
    pause 3
    "images/animation/grope_04.png"
    pause.1
    "images/animation/00.png"
    pause.1
    "images/animation/grope_04.png"
    pause.1
    "images/animation/00.png"
    pause 3
    repeat

image groping_02_blinking: #Animation of Hermione blinking her eyes.
    "images/animation/00.png"
    pause.1
    "images/animation/grope_b_04.png"
    pause.1
    "images/animation/00.png"
    pause 3
    "images/animation/grope_b_04.png"
    pause.1
    "images/animation/00.png"
    pause.1
    "images/animation/grope_b_04.png"
    pause.1
    "images/animation/00.png"
    pause 3
    repeat

### GROPING TITS FULLY CLOTHED ###
image groping_03_ani: #Genie groping Hermione under her skirt. Hermione is facing Genie.
    "images/animation/molest_01.png"
    pause.2
    "images/animation/molest_02.png"
    pause.2
    "images/animation/molest_03.png"
    pause.2
    "images/animation/molest_04.png"
    pause.2
    "images/animation/molest_05.png"
    pause.2
    "images/animation/molest_06.png"
    pause.2
    "images/animation/molest_07.png"
    pause.2
    "images/animation/molest_08.png"
    pause.2
    repeat

### GROPING NAKED TITS ###
image groping_naked_tits_ani: #Genie groping Hermione under her skirt. Hermione is facing Genie.
    "images/animation/tit_molester_naked_01.png"
    pause.2
    "images/animation/tit_molester_naked_02.png"
    pause.2
    "images/animation/tit_molester_naked_03.png"
    pause.2
    "images/animation/tit_molester_naked_04.png"
    pause.2
    "images/animation/tit_molester_naked_05.png"
    pause.2
    "images/animation/tit_molester_naked_06.png"
    pause.2
    "images/animation/tit_molester_naked_07.png"
    pause.2
    "images/animation/tit_molester_naked_08.png"
    pause.2
    repeat
    
image groping_naked_tits_ani_01: #Genie groping Hermione under her skirt. Hermione is facing Genie.
    "images/animation/tit_molester_naked_00_01.png"
    pause.2
    "images/animation/tit_molester_naked_00_02.png"
    pause.2
    "images/animation/tit_molester_naked_00_03.png"
    pause.2
    "images/animation/tit_molester_naked_00_04.png"
    pause.2
    "images/animation/tit_molester_naked_00_05.png"
    pause.2
    "images/animation/tit_molester_naked_00_06.png"
    pause.2
    "images/animation/tit_molester_naked_00_07.png"
    pause.2
    "images/animation/tit_molester_naked_00_08.png"
    pause.2
    repeat


### JERKING OFF ###
image jerking_off_ani: #Genie jerking off in front of top pulled up (shirt + vest) Hermione.
    "images/animation/07_jerking_off_01.png"
    pause.2
    "images/animation/07_jerking_off_02.png"
    pause.2
    "images/animation/07_jerking_off_03.png"
    pause.2
    "images/animation/07_jerking_off_04.png"
    pause.2
    repeat
    
### JERKING OFF 2.0###
# image jerking_off_ani_a: #Genie jerking off in front of top pulled up (shirt + vest) Hermione.
    # "images/animation/07_jerking_off_a_01.png"
    # pause.2
    # "images/animation/07_jerking_off_a_02.png"
    # pause.2
    # "images/animation/07_jerking_off_a_03.png"
    # pause.2
    # "images/animation/07_jerking_off_a_04.png"
    # pause.2
    # repeat
# image jerking_off_ani_b: #Genie jerking off in front of top pulled up (shirt) Hermione.
    # "images/animation/07_jerking_off_b_01.png"
    # pause.2
    # "images/animation/07_jerking_off_b_02.png"
    # pause.2
    # "images/animation/07_jerking_off_b_03.png"
    # pause.2
    # "images/animation/07_jerking_off_b_04.png"
    # pause.2
    # repeat
# image jerking_off_ani_c: #Genie jerking off in front of top pulled up (vest) Hermione.
    # "images/animation/07_jerking_off_c_01.png"
    # pause.2
    # "images/animation/07_jerking_off_c_02.png"
    # pause.2
    # "images/animation/07_jerking_off_c_03.png"
    # pause.2
    # "images/animation/07_jerking_off_c_04.png"
    # pause.2
    # repeat
# image jerking_off_ani_d: #Genie jerking off in front of topless Hermione. 
    # "images/animation/07_jerking_off_d_01.png"
    # pause.2
    # "images/animation/07_jerking_off_d_02.png"
    # pause.2
    # "images/animation/07_jerking_off_d_03.png"
    # pause.2
    # "images/animation/07_jerking_off_d_04.png"
    # pause.2
    # repeat
    
image desk_blinking: #Animation of Hermione blinking her eyes standing at genie's desk.
    "images/animation/00.png"
    pause.1
    "images/animation/grope_04.png"
    pause.1
    "images/animation/00.png"
    pause 3
    "images/animation/grope_04.png"
    pause.1
    "images/animation/00.png"
    pause.1
    "images/animation/grope_04.png"
    pause.1
    "images/animation/00.png"
    pause 3
    repeat
    
   
image groping_ass_ani: #Genie groping Hermione's ass
    "images/animation/06_groping_01.png"
    pause.6
    "images/animation/06_groping_02.png"
    pause.6
    "images/animation/06_groping_03.png"
    pause.6
    "images/animation/06_groping_04.png"
    pause.6
    repeat

image jerking_off_02_ani: #Genie jerking off in front of dancing on a desk Hermione. (Difference is that the dick is aimed higher.)
    "images/animation/06_jerking_01.png"
    pause.2
    "images/animation/06_jerking_02.png"
    pause.2
    "images/animation/06_jerking_03.png"
    pause.2
    "images/animation/06_jerking_04.png"
    pause.2
    repeat

image jerking_off_03_ani: #SNape jerking off in front of dancing on a desk Hermione.
    "images/animation/10_jerking_01.png"
    pause.2
    "images/animation/10_jerking_02.png"
    pause.2
    "images/animation/10_jerking_03.png"
    pause.2
    "images/animation/10_jerking_04.png"
    pause.2
    repeat

### CUM ###
image jerking_off_cum_ani: #Sperm.
    "images/animation/08_cum_01.png"
    pause.1
    "images/animation/08_cum_02.png"
    pause.1
    "images/animation/08_cum_03.png"
    pause.1
    "images/animation/08_cum_04.png"
    pause.1
    "images/animation/08_cum_05.png"
    pause.1
    "images/animation/08_cum_06.png"
    pause.1
    "images/animation/08_cum_07.png"
    pause.1
    "images/animation/08_cum_08.png"
    pause.1
    "images/animation/08_cum_09.png"
    pause.1
    "images/animation/08_cum_10.png"
    pause.1
    "images/animation/08_cum_11.png"
    pause.1
    "images/animation/08_cum_12.png"
    pause.1
    "images/animation/08_cum_13.png"
    pause 3
    "images/animation/08_cum_14.png"
    pause.1
    "images/animation/08_cum_15.png"
    pause.1
    "images/animation/08_cum_16.png"
    pause.1
    repeat

### CUM_03 ###
image snape_cum_01: #Snape cumming on Hermione after she is done striping.
    "images/animation/11_cum_01.png"
    pause.1
    "images/animation/11_cum_02.png"
    pause.1
    "images/animation/11_cum_03.png"
    pause.1
    "images/animation/11_cum_04.png"
    pause.1
    "images/animation/11_cum_05.png"
    pause.1
    "images/animation/11_cum_06.png"
    pause.1
    "images/animation/11_cum_07.png"
    pause.1
    "images/animation/11_cum_08.png"
    pause.1
    "images/animation/11_cum_09.png"
    pause.1
    "images/animation/11_cum_10.png"
    pause.1
    "images/animation/11_cum_11.png"
    pause.1
    "images/animation/11_cum_12.png"
    pause.1
    "images/animation/11_cum_13.png"
    pause.1
    "images/animation/11_cum_14.png"
    pause.1
    "images/animation/11_cum_15.png"
    pause.1
    "images/animation/11_cum_16.png"
    pause.1
    "images/animation/11_cum_17.png"
    pause.1
    "images/animation/11_cum_18.png"
    pause 2
    "images/animation/11_cum_19.png"
    pause.1
    "images/animation/11_cum_20.png"
    pause.1
    "images/animation/00.png"
    pause.5
    repeat



