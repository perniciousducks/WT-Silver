...
label update_quest_items:
    ...
    if sealed_scroll_quest_ITEM.unlocked and not tentacle_sample:
        if sealed_scroll_quest_ITEM not in gen_quest_items_list:
            $ gen_quest_items_list.append(sealed_scroll_quest_ITEM)
            $ sealed_scroll_quest_ITEM.number = 1 #Makes it clickable in the inventory UI.
    else:
        if sealed_scroll_quest_ITEM in gen_quest_items_list:
            pass # Don't remove the item, the player may think it can no longer be used
            #$ gen_quest_items_list.remove(sealed_scroll_quest_ITEM)

    #Puzzle Box
    if puzzle_box_quest_ITEM.unlocked and unlocked_7th == False:
        if puzzle_box_quest_ITEM not in gen_quest_items_list:
            $ gen_quest_items_list.append(puzzle_box_quest_ITEM)
            $ puzzle_box_quest_ITEM.number = 1 #Makes it clickable in the inventory UI.
    else:
        if puzzle_box_quest_ITEM in gen_quest_items_list:
            $ gen_quest_items_list.remove(puzzle_box_quest_ITEM)
    ...
    if collar == 0 and her_whoring >= 24:
        if collar_quest_ITEM not in gen_quest_items_list:
            $ gen_quest_items_list.append(collar_quest_ITEM)
            $ collar_quest_ITEM.number = 1 #Makes it clickable in the gift UI.
    else:
        if collar_quest_ITEM in gen_quest_items_list:
            $ gen_quest_items_list.remove(collar_quest_ITEM)
